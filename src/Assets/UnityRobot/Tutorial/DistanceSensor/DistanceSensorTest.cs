...
public class DistanceSensorTest : MonoBehaviour
{
	...
	void OnGUI()
	{
		...
		if(robot.Connected == true)
		{
			...
			guiRect.width = 300;
			GUI.Label(guiRect, string.Format("Analog value: {0:d}", sensor.Value));
			guiRect.y += (guiRect.height + 5);

			guiRect.width = 300;
			GUI.Label(guiRect, string.Format("Distance: {0:d}mm", (int)sensor.Distance));
			guiRect.y += (guiRect.height + 5);
		}
		else
		{
