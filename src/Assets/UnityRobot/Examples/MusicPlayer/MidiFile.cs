    ...
    public bool ChangeSound(MidiSoundOptions options, string destfile)
    {
        ...
        for (int tracknum = 0; tracknum < tracks.Count; tracknum++)
        {
            ...
            if (options.instruments == null)
                Debug.LogWarning("option instruments is null");
 ...
 */
    ...
    * - csc MidiFile.cs TimeSignature.cs MyAssert.cs
    * - MidiFile.exe file.mid
    *
    * Side note: The SheetMusicOptions.key is an object (rather than KeySignature)
    * so that we can build MidiFile.exe with just 3 csharp files.
    */
    ...
    //protected override void OnInitialized(SiCiControl sender, object[] eventParams)
    //{
    //}
